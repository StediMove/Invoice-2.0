...
const plans = [
  {
    ...
    includes: [
      "Free includes:",
      "5 AI-generated invoices per month",
  ...
  {
    name: "Professional", 
    description: "Best value for growing businesses that need more features",
    price: 9.90,
    yearlyPrice: 90,
    buttonText: "Get started",
...
export default function PricingSection4() {
  ...
  const handleSubscribe = async (plan: typeof plans[0]) => {
    ...
    // Only handle Professional plan subscription
    if (plan.name === 'Professional') {
      const userEmail = user.email || '';
      
      // Use Stripe payment links from configuration with user email pre-filled
      const monthlyLink = `${STRIPE_CONFIG.paymentLinks.professional.monthly}?prefilled_email=${encodeURIComponent(userEmail)}`;
      const yearlyLink = `${STRIPE_CONFIG.paymentLinks.professional.yearly}?prefilled_email=${encodeURIComponent(userEmail)}`;
      
      const checkoutUrl = isYearly ? yearlyLink : monthlyLink;
  ...
  return (
    ...
    >
      ...
      <article className="text-center mb-6 pt-32 max-w-3xl mx-auto space-y-2 relative z-50">
        ...
        >
          Trusted by thousands of businesses. Explore which option is right for you.
        </TimelineContent>
      ...
      <div className="grid md:grid-cols-2 max-w-4xl gap-6 py-6 mx-auto px-4 justify-center">
